#!/usr/bin/env python3
"""
SVA gRPC Server - HuggingFace Compliant (Fixed File Path Issues)
...
from typing import Dict, Any, Optional
                        ...
                        if 'bbox' in text_item and text_item['bbox']:
                            ...
                            if isinstance(bbox_data, list) and len(bbox_data) >= 4:
                                ...
                                bbox.height = bbox_data.get('height', 0)d
...
def resolve_video_path(video_path: str) -> Optional[str]:
    """
    Smart video path resolution - handles multiple path formats and locations
    """
    logger.info(f"📁 Resolving video path: {video_path}")
    ...
    search_paths = [
        ...
        os.path.join('..', 'data', 'videos', video_path),
        os.path.join('data', 'videos', video_path),
        os.path.join('test_videos', video_path),
        os.path.join('..', 'test_videos', video_path),
        
        # Try with current working directory
        os.path.join(os.getcwd(), video_path),
        os.path.join(os.getcwd(), '..', 'data', 'videos', video_path),
        
        # Try absolute paths
        os.path.join('/home/user/SVA/data/videos', video_path),
        os.path.join('/home/user/SVA/backend/test_videos', video_path),
        
        # If video_path contains path separators, try it as relative to different bases
        os.path.join('..', video_path) if '/' in video_path else None,
    ]
    ...
    for path in search_paths:
        ...
        if os.path.exists(abs_path) and os.path.isfile(abs_path):
            logger.info(f"✅ Found video at: {abs_path}")
            return abs_path
    
    # If nothing found, log all attempted paths
    logger.error(f"❌ Video file not found. Searched in:")
    for path in search_paths:
...
class SVAServiceImpl(sva_pb2_grpc.SVAServiceServicer):
    ...
    async def AnalyzeVideo(self, request, context):
        ...
        try:
            ...
            # Resolve video path using smart path resolution
            resolved_path = resolve_video_path(request.video_path)
...
async def serve():
    ...
    logger.info(f"🚀 Starting SVA gRPC Server (Fixed) on {listen_addr}")
    logger.info("📱 Frontend Requirements: ✅ gRPC Communication")
    logger.info("🤖 Backend: ✅ HuggingFace Compliant")
    logger.info("🔧 File Path Resolution: ✅ Smart Path Finding")
